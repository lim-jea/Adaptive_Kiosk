...
> 검토 로그: [01_REVIEW_LOG.md](./01_REVIEW_LOG.md)
...
| CHANGE-009 | **해소** | (기존 `create_data/build_dataset.py` 다중 라인 부재) | Phase 1.4 자체 합성에서 G 가설(다중 라인 30~50% + Bread Basket 페어 분포 매핑)로 직접 주입 | — | **CLOSED 2026-05-04** — [06_NOTEBOOK_PLAN.md](./06_NOTEBOOK_PLAN.md) 노트북 03에서 처리 |
...
| CHANGE-017 | 보류-내부 | `create_data/recommendation_test/generate_synth_v2.py` | v1 합성 데이터의 단일-메뉴 도배(말차 9.84%) + 컨텍스트 분리력 부족(unique top1 3/40) 해결. period × 메뉴 + (성별,연령) × 메뉴 prior 명시 + softmax cap 6%. backend `_hour_to_period` 와 동일한 5-period 사용 | 합성 데이터 품질 | **CANDIDATE 2026-05-05** — 격리 검증 통과 (max share 5.15%, HHI 0.0457, unique top1 18/40). [08_TODAY_WORKLOG_2026-05-05.md](./08_TODAY_WORKLOG_2026-05-05.md) |
