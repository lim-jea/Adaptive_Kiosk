...
[음성 조각(audio_segments)]
- 조합형 응답(주문 요약/가격 안내/옵션 선택 확정 등)에서 가능하면 audio_segments를 함께 채운다.
- audio_segments는 반드시 컨텍스트에 제공된 템플릿 parts/조각 목록에 있는 문자열만 정확히 사용한다(공백 포함).
- 정확히 구성할 자신이 없으면 audio_segments는 null로 둔다(오히려 잘못된 조각은 금지).
